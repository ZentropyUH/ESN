...
class InputMatrix(keras.initializers.Initializer):
    ...
    def __call__(self, shape, dtype=tf.float64, **kwargs):
        ...
        indexes_nonzero = np.zeros(
            (rows * inputs_per_node + q_flag, 2), dtype=np.int32
        )

        for i in range(0, rows):
            for j in range(0, inputs_per_node + q_flag):
                indexes_nonzero[i * inputs_per_node + j, :] = [
                    i,
                    i * inputs_per_node + j,
                ]

        values = np.random.uniform(
            -self.sigma, self.sigma, size=rows * inputs_per_node + q_flag
        )
...
class RegularOwn(keras.initializers.Initializer):
    ...
    def __call__(self, shape, dtype=tf.float32, **kwargs):
        ...
        if self.ones:
            values = np.ones(rows * degree)
        else:
            values = np.random.uniform(-self.sigma, self.sigma, rows * degree)
        ...
        ans = tf.sparse.to_dense(sparse_w)
...
class RegularNX(keras.initializers.Initializer):
    ...
    def __call__(self, shape, dtype=tf.float32, **kwargs):
        ...
        if not self.ones:  # Make this more efficient
            ...
            # Go back to sparse matrix to calculate the spectral radius efficiently
