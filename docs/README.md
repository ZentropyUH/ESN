...
Basic hyper‑parameter optimisation is handled by `run_hpo`. A full example is provided in {doc}`hyperparameter_optimization`, but the general workflow is:
...
For advanced configuration options and additional examples see {doc}`advanced_usage`.
